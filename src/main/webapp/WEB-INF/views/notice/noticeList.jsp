...
<div style="height: 400px; border-top: solid;">
	...
	<div align="center" style="height: auto; text-align: center">
		<div class="wrapper">
			...
			<div align="center">
					...
					<table border="0.2" class="NoticeTable" style="margin-top: 1em">
						...
						<tbody>
							<c:forEach var="row" items="${noList }">
								<tr>
									<td class="idx" id="BOARD_NO" style="text-align: center;">${row.BOARD_NO }</td>
									<td align="left" class="subject">
										<!-- <c:if test="${board.comment >= 10}"><img src="<%=request.getContextPath()%>/img/hit.jpg" /></c:if> -->
										<a href="#this" name="subject">${row.NOTICE_SUBJECT }</a> <input
										type="hidden" id="BOARD_NO" value="${row.BOARD_NO }">
									</td>
									<td class="writer" style="text-align: center;">
										<%--  <c:choose> 
									 <c:when test="${fn:length(list) > 0}">  --%> <strong>${row.NOTICE_WRITER }</strong>
										<%-- </c:when> 
									 <c:otherwise>.</c:otherwise>
								</c:choose> --%>
									</td>
									<td class="hitcount" style="text-align: center;">${row.NOTICE_COUNT }</td>
									<td class="writeDate" style="text-align: center;">${row.NOTICE_DATE }</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
				...
				<div class="navBar">
					...
					<input type="text" name="isSearch" style="width: 20em; position: relative; right: 17em; bottom: 2.5em;"
						> <input type="submit"  class="keyword" style="position: relative; bottom: 2.5em; right: 17em"
						value="검색">
